...
- **Fotos huérfanas en Storage** (visible desde el rediseño del alta de producto, 2026-08-14, pero venía de antes): al quitar una foto de un producto se borra la fila de `product_images` pero **no** el objeto del bucket `products`. Se acumula basura que se paga. No urgente; requiere parsear la ruta desde la URL pública para poder borrarla. **Solo afecta al bucket `products`**: el bucket `avatars` (2026-08-28) ya borra la foto anterior al reemplazarla o quitarla — ver `deleteStoredAvatar()` en `js/perfil.js`, sirve de modelo para cuando se arregle el de productos.
- **Baja de cuenta = pedido manual, no borrado automático** (desde el rediseño de "Información de tu perfil", 2026-08-28): "Eliminar mi cuenta" abre un ticket de soporte para que lo procese un admin. Borrar el usuario de `auth.users` de verdad necesita la service role key, que no puede vivir en el navegador — haría falta una Edge Function. Hoy **nadie está procesando esos tickets** (no hay admin asignado todavía, ver más abajo).
