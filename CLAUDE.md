...
- **Resuelto 2026-09-09** — se asignó el rol `admin` (`raw_app_meta_data.role`) a tres cuentas reales en producción: `keilakrausnl08@gmail.com`, `alganarasberenice@gmail.com` (antes tenía `role: "vendedor"` — quedó reemplazado por `admin`, no puede tener los dos a la vez) y `bianberayra@gmail.com`. El JWT de cada una se actualiza recién en su próximo login. Para sumar otro admin: dashboard de Supabase → Authentication → Users → `raw_app_meta_data` → `{"role": "admin"}`.
