...
- **Baja de cuenta: falta probarla con una cuenta real** (2026-08-28): la Edge Function `delete-account` ya está desplegada y activa, y se verificó que rechaza llamadas sin sesión (401) y que responde el preflight de CORS. Lo que **no** se probó es el camino feliz — hacerlo borra una cuenta de verdad, es irreversible. Conviene probarlo con una cuenta de descarte antes de confiar en él. Ojo con las dos guardas: si la cuenta tiene un comercio o pedidos en `paid`/`shipped`/`ready_for_pickup`, la función responde 409 y **no** borra (a propósito: `stores.owner_id` es CASCADE y se llevaría la tienda entera).
