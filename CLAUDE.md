...
- **Falta desplegar la Edge Function `delete-account`** (2026-08-28): el código está versionado en `supabase/functions/delete-account/index.ts` pero **todavía no está desplegada** en el proyecto de Supabase. Hasta que se despliegue, "Eliminar mi cuenta" sigue cayendo al pedido manual por ticket de soporte (respaldo automático ante 404, ver `requestDeletionByTicket()` en `js/perfil.js`), y esos tickets no los mira nadie porque no hay admin asignado. Desplegar con `npx supabase functions deploy delete-account` (requiere estar linkeado al proyecto `otzhdwuaffcplrveuadc`).
