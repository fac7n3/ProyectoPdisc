    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords }) => {
        try {
          ...
          if (data?.address) {
            // Misma salida que elegir una sugerencia: OSM tampoco trae la altura en
            // el reverse, así que queda la calle con el cursor al final para
            // que la persona escriba el número.
            pickAddressSuggestion(data);
          } else {
  ...
  field.inputs(profileData).forEach((spec) => {
    if (spec.el === "select") {
      ...
      const dd = buildDropdown({
        // Las opciones vienen ya como {value, label} (característica de
        // país, donde el label muestra bandera + nombre).
