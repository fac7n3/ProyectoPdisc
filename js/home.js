...
import { supabase, getPanelAccess, HOME_INTENT_KEY } from './auth-utils.js';
 ...
 * usuario, 2026-09-16. La puerta de vuelta al home es el logo del navbar (ver
 * cameToHomeOnPurpose).
 *
 * Quien ADEMÁS es admin no se redirige: tiene dos paneles y elegir uno por su
 * cuenta sería adivinar (ver hasSellerPanel en auth-utils.js). Se queda en el
...
function renderPanelAction({ isAdmin, seller, role }) {
  ...
  if (!isAdmin || !seller) {
    ...
    link.href = isAdmin ? './admin.html' : './vender.html';
    return;
  }
  ...
  [
    { href: './vender.html', icon: seller === 'profesional' ? 'fa-solid fa-screwdriver-wrench' : 'fa-solid fa-shop', label: PANEL_LABELS[seller] },
