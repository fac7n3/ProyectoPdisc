import{c as e,l as t,o as n,r,s as i}from"./cart-utils-DUumeUPs.js";function a(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}function o(e){let t=e.id,r=e.dataset.price===void 0?n(e.querySelector(`.product-card__price`)?.textContent||`0`):Number(e.dataset.price),i=a(e.querySelector(`.product-card__name`)?.textContent?.trim()||`Producto`),o=a(e.querySelector(`.product-card__shop`)?.textContent?.trim()?.replace(/^\s*/,``)||`Tienda`),s=a(e.querySelector(`.product-card__price`)?.textContent||`$0`),c=a(e.querySelector(`.product-card__price-old`)?.textContent||``),l=a(e.querySelector(`.product-card__discount`)?.textContent||``),u=encodeURI(e.querySelector(`.product-card__image img`)?.getAttribute(`src`)||``),d=a(e.querySelector(`.product-card__image img`)?.getAttribute(`alt`)||i),f=a(e.querySelector(`.product-card__shipping`)?.textContent?.trim()||``),p=e.querySelector(`.product-card__stars`),m=p?.querySelectorAll(`.fa-star:not(.empty)`).length||0,h=p?.querySelectorAll(`.fa-star-half-stroke`).length||0,g=p?.querySelectorAll(`.fa-star.empty, .fa-regular.fa-star.empty`).length||0,_=a(e.querySelector(`.product-card__rating-count`)?.textContent?.replace(/[()]/g,``)||`0`),v=(e.dataset.category||``).split(` `),y=e.querySelector(`.product-card__badge`);return{id:t,price:r,name:i,shop:o,priceText:s,priceOldText:c,discountText:l,imgSrc:u,imgAlt:d,shippingText:f,fullStars:m,halfStars:h,emptyStars:g,ratingCount:_,categories:v,badgeText:a(y?.textContent?.trim()||``),badgeType:y?.classList.contains(`product-card__badge--envio`)?`envio`:y?.classList.contains(`product-card__badge--oferta`)?`descuento`:``,stock:(t||``).split(``).reduce((e,t)=>e+t.charCodeAt(0),0)%40+5}}function s(e,t,n){let r=``;for(let t=0;t<e;t++)r+=`<i class="fa-solid fa-star"></i>`;for(let e=0;e<t;e++)r+=`<i class="fa-solid fa-star-half-stroke"></i>`;let i=5-e-t;for(let e=0;e<i;e++)r+=`<i class="fa-regular fa-star empty"></i>`;return r}function c(e){return e<=10?{text:`¡Últimas ${e} unidades!`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=25?{text:`Quedan ${e} unidades`,cssClass:`ok`,fillClass:`pm-stock__fill--mid`}:{text:`Disponible (${e} unidades)`,cssClass:`ok`,fillClass:`pm-stock__fill--high`}}function l(e,t){let n=document.querySelectorAll(`.product-card`),r=[];return n.forEach(n=>{if(n.id===e)return;let i=(n.dataset.category||``).split(` `);t.some(e=>e&&i.includes(e))&&r.push(n)}),r.length<4&&n.forEach(t=>{t.id!==e&&!r.includes(t)&&r.length<6&&r.push(t)}),r.slice(0,6)}function u(e){let t=c(e.stock),n=e.shop.replace(/^[\s\W]*/,``).charAt(0).toUpperCase();e.shop.replace(/^[\s]*/,``).replace(/^\s*\S+\s*/,``);let r=l(e.id,e.categories),i=``;r.forEach(e=>{let t=a(e.querySelector(`.product-card__name`)?.textContent?.trim()||``),n=a(e.querySelector(`.product-card__price`)?.textContent||``),r=encodeURI(e.querySelector(`.product-card__image img`)?.getAttribute(`src`)||``),o=a(e.id||``);i+=`
      <div class="pm-related-card" data-related-id="${o}" tabindex="0" role="button" aria-label="Ver ${t}">
    ...
    `});let o=``;e.badgeText&&(o=`<span class="pm-gallery__badge pm-gallery__badge--${e.badgeType||`descuento`}">${e.badgeText}</span>`);let u=``;e.discountText&&(u=`<span class="pm-discount-tag">${e.discountText}</span>`);let d=``;return e.priceOldText&&(d=`<span class="pm-price-old">${e.priceOldText}</span>`),`
