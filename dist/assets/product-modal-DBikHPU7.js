import{s as e}from"./speed-insights-CGQ9u1QN.js";import{S as t,b as n,c as r,m as i,o as a,s as o,v as s,y as c}from"./dropdown-DiNs99rX.js";import{i as l,r as u}from"./reviews-utils-8R-_cou0.js";function d(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}async function f(t){let[{data:n,error:r},i,{data:{session:o}}]=await Promise.all([e.from(`products`).select(`id, title, description, price, compare_at_price, offer_expires_at, stock, image_url, stores(id, name, owner_id, delivery_fee, free_shipping_threshold), product_images(url, position), product_variants(id, name, price, stock)`).eq(`id`,t).single(),u(`product`,t),e.auth.getSession()]);if(r||!n)throw r||Error(`Producto no encontrado`);let s=new Date().toISOString().slice(0,10),c=!!(n.offer_expires_at&&n.offer_expires_at<s),l=!!(n.compare_at_price&&n.compare_at_price>n.price&&!c),f=l?Math.round((1-n.price/n.compare_at_price)*100):0,p=n.stores||{},m=(n.product_images||[]).slice().sort((e,t)=>e.position-t.position),h=[n.image_url||`/img/no-image.svg`,...m.map(e=>e.url)],g=p.free_shipping_threshold!=null&&n.price>=p.free_shipping_threshold,_;_=p.delivery_fee==null?`El costo de envío se calcula en el carrito, según el comercio.`:g||p.delivery_fee===0?`Envío gratis en este comercio.`:`Envío: ${a(p.delivery_fee)}`+(p.free_shipping_threshold?` (gratis desde ${a(p.free_shipping_threshold)})`:``);let v=``,y=``;l?(v=`-${f}%`,y=`descuento`):g&&(v=`Envío gratis`,y=`envio`);let b=i.average,x=i.count,S=b?Math.floor(b):0,C=b&&b-Math.floor(b)>=.5?1:0,w=Math.max(0,5-S-C),T=!!(o?.user?.id&&p.owner_id&&o.user.id===p.owner_id);return{id:n.id,isOwner:T,price:n.price,name:d(n.title||`Producto`),description:d(n.description||``),shop:d(p.name||`Comercio`),shopId:p.id||null,priceText:a(n.price),priceOldText:l?a(n.compare_at_price):``,discountText:l?`-${f}%`:``,images:h,imgSrc:h[0],imgAlt:d(n.title||`Producto`),shippingText:_,fullStars:S,halfStars:C,emptyStars:w,ratingCount:x,hasRating:x>0,badgeText:v,badgeType:y,stock:n.stock??0,variants:n.product_variants||[]}}function p(e,t,n){let r=``;for(let t=0;t<e;t++)r+=`<i class="fa-solid fa-star"></i>`;for(let e=0;e<t;e++)r+=`<i class="fa-solid fa-star-half-stroke"></i>`;let i=5-e-t;for(let e=0;e<i;e++)r+=`<i class="fa-regular fa-star empty"></i>`;return r}function m(e){return e<=0?{text:`Sin stock disponible`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=10?{text:`¡Últimas ${e} unidades!`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=25?{text:`Quedan ${e} unidades`,cssClass:`ok`,fillClass:`pm-stock__fill--mid`}:{text:`Disponible (${e} unidades)`,cssClass:`ok`,fillClass:`pm-stock__fill--high`}}function h(e,t){let n=document.querySelectorAll(`.product-card`),r=[];return n.forEach(n=>{if(n.id===e)return;let i=(n.dataset.category||``).split(` `);t.some(e=>e&&i.includes(e))&&r.push(n)}),r.length<8&&n.forEach(t=>{t.id!==e&&!r.includes(t)&&r.length<12&&r.push(t)}),r.slice(0,12)}function g(e){return`${window.location.pathname.includes(`/pages/`)?`./`:`./pages/`}comercio.html?id=${encodeURIComponent(e)}`}function _(){return`
  ...
  `}var b=null,x=null,S=null;function C(e){let t=(history.state?.pmDepth||0)+1;history.pushState({pmProduct:e,pmDepth:t},``,location.href)}function w(){let e=history.state?.pmDepth||0;e>0?history.go(-e):E()}window.addEventListener(`popstate`,e=>{let t=e.state?.pmProduct,n=t&&document.getElementById(t);n?T(n,{skipHistoryPush:!0}):b&&E()});async function T(e,{skipHistoryPush:t=!1}={}){if(!e.id){console.error(`Se intentó abrir el modal de un producto sin id real (UUID)`);return}b&&E(),x=document.activeElement;let n=e.id,r=(e.dataset.category||``).split(` `);t||C(n);let i=document.createElement(`div`);i.className=`product-modal-overlay`,i.id=`pm-overlay`,i.innerHTML=_(),document.body.appendChild(i),b=i;let a=window.innerWidth-document.documentElement.clientWidth;document.documentElement.style.setProperty(`--scrollbar-width`,a+`px`),document.body.classList.add(`modal-open`),i.offsetHeight,requestAnimationFrame(()=>i.classList.add(`is-open`)),i.addEventListener(`click`,e=>{(e.target.closest(`#pm-close-back`)||e.target.closest(`#pm-close-btn`))&&w()}),S=e=>{e.key===`Escape`&&w()},document.addEventListener(`keydown`,S);try{let e=await f(n);if(e.categories=r,b!==i)return;i.innerHTML=y(e),D(i,e);let t=i.querySelector(`.pm-stock__fill`);if(t){let n=e.stock<=0?`0%`:``;t.style.width=`0%`,requestAnimationFrame(()=>{requestAnimationFrame(()=>{t.style.width=n})})}}catch(t){if(console.error(`Error al cargar el producto en el modal:`,t),b!==i)return;i.innerHTML=v(),i.querySelector(`#pm-retry-btn`)?.addEventListener(`click`,()=>T(e))}}function E(){if(!b)return;b.classList.remove(`is-open`),document.body.classList.remove(`modal-open`),document.documentElement.style.removeProperty(`--scrollbar-width`);let e=b;b=null,S&&=(document.removeEventListener(`keydown`,S),null),setTimeout(()=>{e.remove()},400),x&&=(x.focus(),null)}function D(e,a){let u=e.querySelector(`.product-modal`),d=e.querySelector(`#pm-gallery`);d?.addEventListener(`click`,e=>{e.target.closest(`.pm-gallery__fav`)||e.target.closest(`.pm-gallery__nav`)||d.classList.toggle(`is-zoomed`)});let f=e.querySelector(`#pm-gallery-img`),p=Array.from(e.querySelectorAll(`.pm-thumb`)),m=0;function h(e){let t=a.images.length;if(t===0)return;m=(e%t+t)%t;let n=a.images[m];f&&n&&(f.src=encodeURI(n)),p.forEach((e,t)=>e.classList.toggle(`is-active`,t===m))}if(p.forEach((e,t)=>{e.addEventListener(`click`,()=>h(t))}),e.querySelector(`#pm-gallery-prev`)?.addEventListener(`click`,()=>h(m-1)),e.querySelector(`#pm-gallery-next`)?.addEventListener(`click`,()=>h(m+1)),a.shopId){let t=()=>{window.location.href=g(a.shopId)},n=e.querySelector(`#pm-shop-link`);n?.addEventListener(`click`,t),n?.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),t())}),e.querySelector(`#pm-view-shop-btn`)?.addEventListener(`click`,t)}let _=e.querySelector(`#pm-fav-btn`);if(_&&a.id){let e=_.querySelector(`i`),t=!1;r().then(n=>{t=n.includes(a.id),_.classList.toggle(`is-active`,t),e.classList.toggle(`fa-solid`,t),e.classList.toggle(`fa-regular`,!t)}),_.addEventListener(`click`,async()=>{let r=t;t=!r,_.classList.toggle(`is-active`,t),e.classList.toggle(`fa-solid`,t),e.classList.toggle(`fa-regular`,!t),c(t?`Agregado a favoritos`:`Eliminado de favoritos`,t?`success`:`default`);try{await n(a.id,r)}catch(e){console.error(`Error al actualizar favoritos:`,e)}})}let v=e.querySelector(`#pm-qty-value`),y=e.querySelector(`#pm-qty-minus`),b=e.querySelector(`#pm-qty-plus`);function x(){let e=parseInt(v.value,10)||1;y.disabled=e<=1,b.disabled=e>=a.stock}y?.addEventListener(`click`,()=>{let e=parseInt(v.value,10)||1;e>1&&(v.value=e-1),x()}),b?.addEventListener(`click`,()=>{let e=parseInt(v.value,10)||1;e<a.stock&&(v.value=e+1),x()}),v?.addEventListener(`change`,()=>{let e=parseInt(v.value,10)||1;e=Math.max(1,Math.min(e,a.stock)),v.value=e,x()});let S=e.querySelector(`#pm-add-cart`);S?.addEventListener(`click`,()=>{let e=parseInt(v?.value,10)||1,n=i(a.priceOldText),r=o(),l=r.find(e=>e.id===a.id);l?(l.qty+=e,l.selected=!0):r.push({id:a.id,name:a.name,shop:a.shop,price:a.price,priceOld:n||null,image:a.imgSrc,qty:e,selected:!0}),s(r),t(),S.classList.add(`pm-btn--added`);let u=S.innerHTML;S.innerHTML=`<i class="fa-solid fa-check"></i> ¡Agregado!`,setTimeout(()=>{S.classList.remove(`pm-btn--added`),S.innerHTML=u},1800),c(`${a.name} agregado al carrito (x${e})`,`success`)}),e.querySelector(`#pm-buy-now`)?.addEventListener(`click`,()=>{let e=parseInt(v?.value,10)||1,n=i(a.priceOldText),r=o(),c=r.find(e=>e.id===a.id);c?(c.qty+=e,c.selected=!0):r.push({id:a.id,name:a.name,shop:a.shop,price:a.price,priceOld:n||null,image:a.imgSrc,qty:e,selected:!0}),s(r),t(),E();let l=window.location.pathname.includes(`/pages/`);window.location.href=l?`./carrito.html`:`./pages/carrito.html`}),e.querySelector(`#pm-share-btn`)?.addEventListener(`click`,async()=>{let e={title:a.name,text:`Mirá ${a.name} en Baradero Local por ${a.priceText}`,url:window.location.href};try{navigator.share?await navigator.share(e):(await navigator.clipboard.writeText(`${e.text} — ${e.url}`),c(`¡Link copiado al portapapeles!`,`success`))}catch(e){if(e.name!==`AbortError`)try{await navigator.clipboard.writeText(window.location.href),c(`¡Link copiado!`,`success`)}catch{}}});let C=e.querySelectorAll(`.pm-tab`),w=e.querySelectorAll(`.pm-tab-content`),D=!1;C.forEach(t=>{t.addEventListener(`click`,()=>{C.forEach(e=>{e.classList.remove(`is-active`),e.setAttribute(`aria-selected`,`false`)}),w.forEach(e=>e.classList.remove(`is-visible`)),t.classList.add(`is-active`),t.setAttribute(`aria-selected`,`true`);let n=t.dataset.tab,r=e.querySelector(`[data-tab-content="${n}"]`);if(r&&r.classList.add(`is-visible`),n===`reviews`&&!D){D=!0;let t=e.querySelector(`#pm-reviews-container`);t&&l(t,`product`,a.id,{hideForm:a.isOwner})}})}),e.querySelectorAll(`.pm-related-card`).forEach(e=>{let t=()=>{let t=e.dataset.relatedId,n=document.getElementById(t);n&&(E(),setTimeout(()=>T(n),450))};e.addEventListener(`click`,t),e.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),t())})});let O=e.querySelector(`.pm-related__scroll`);if(O){let t=e.querySelector(`.pm-related__nav--prev`),n=e.querySelector(`.pm-related__nav--next`),r=e=>{let t=O.querySelector(`.pm-related-card`),n=t?t.getBoundingClientRect().width+14:174;O.scrollBy({left:e*n*2,behavior:`smooth`})},i=()=>{let e=O.scrollWidth-O.clientWidth,r=O.scrollLeft<=4,i=O.scrollLeft>=e-4;t&&(t.disabled=r),n&&(n.disabled=e<=4||i)};t?.addEventListener(`click`,()=>r(-1)),n?.addEventListener(`click`,()=>r(1)),O.addEventListener(`scroll`,i,{passive:!0}),i()}let k=u.querySelectorAll(`button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])`),A=k[0],j=k[k.length-1];u.addEventListener(`keydown`,e=>{e.key===`Tab`&&(e.shiftKey?document.activeElement===A&&(e.preventDefault(),j.focus()):document.activeElement===j&&(e.preventDefault(),A.focus()))}),setTimeout(()=>A?.focus(),100)}function O(){document.querySelectorAll(`.products__grid`).forEach(e=>{e.addEventListener(`click`,e=>{if(e.target.closest(`.product-card__add, .product-card__wishlist`)||e.target.closest(`a[href]`))return;let t=e.target.closest(`.product-card`);t&&(e.preventDefault(),T(t))})})}typeof window<`u`&&(window.initProductModal=O,window.openProductModal=T,window.closeProductModal=E);