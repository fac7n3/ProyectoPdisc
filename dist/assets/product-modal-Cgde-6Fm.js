import{o as e}from"./speed-insights-0RjR2FyT.js";import{C as t,E as n,S as r,f as i,m as a,p as o,v as s,w as c}from"./nav-utils-3h3M3vQC.js";import{n as l,r as u}from"./reviews-utils-BoX9HpFp.js";function d(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}async function f(t){let[{data:n,error:r},a]=await Promise.all([e.from(`products`).select(`id, title, description, price, compare_at_price, offer_expires_at, stock, image_url, stores(id, name, delivery_fee, free_shipping_threshold), product_images(url, position), product_variants(id, name, price, stock)`).eq(`id`,t).single(),l(`product`,t)]);if(r||!n)throw r||Error(`Producto no encontrado`);let o=new Date().toISOString().slice(0,10),s=!!(n.offer_expires_at&&n.offer_expires_at<o),c=!!(n.compare_at_price&&n.compare_at_price>n.price&&!s),u=c?Math.round((1-n.price/n.compare_at_price)*100):0,f=n.stores||{},p=(n.product_images||[]).slice().sort((e,t)=>e.position-t.position),m=[n.image_url||`/img/no-image.svg`,...p.map(e=>e.url)],h=f.free_shipping_threshold!=null&&n.price>=f.free_shipping_threshold,g;g=f.delivery_fee==null?`El costo de envío se calcula en el carrito, según el comercio.`:h||f.delivery_fee===0?`Envío gratis en este comercio.`:`Envío: ${i(f.delivery_fee)}`+(f.free_shipping_threshold?` (gratis desde ${i(f.free_shipping_threshold)})`:``);let _=``,v=``;c?(_=`-${u}%`,v=`descuento`):h&&(_=`Envío gratis`,v=`envio`);let y=a.average,b=a.count,x=y?Math.floor(y):0,S=y&&y-Math.floor(y)>=.5?1:0,C=Math.max(0,5-x-S);return{id:n.id,price:n.price,name:d(n.title||`Producto`),description:d(n.description||``),shop:d(f.name||`Comercio`),shopId:f.id||null,priceText:i(n.price),priceOldText:c?i(n.compare_at_price):``,discountText:c?`-${u}%`:``,images:m,imgSrc:m[0],imgAlt:d(n.title||`Producto`),shippingText:g,fullStars:x,halfStars:S,emptyStars:C,ratingCount:b,hasRating:b>0,badgeText:_,badgeType:v,stock:n.stock??0,variants:n.product_variants||[]}}function p(e,t,n){let r=``;for(let t=0;t<e;t++)r+=`<i class="fa-solid fa-star"></i>`;for(let e=0;e<t;e++)r+=`<i class="fa-solid fa-star-half-stroke"></i>`;let i=5-e-t;for(let e=0;e<i;e++)r+=`<i class="fa-regular fa-star empty"></i>`;return r}function m(e){return e<=0?{text:`Sin stock disponible`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=10?{text:`¡Últimas ${e} unidades!`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=25?{text:`Quedan ${e} unidades`,cssClass:`ok`,fillClass:`pm-stock__fill--mid`}:{text:`Disponible (${e} unidades)`,cssClass:`ok`,fillClass:`pm-stock__fill--high`}}function h(e,t){let n=document.querySelectorAll(`.product-card`),r=[];return n.forEach(n=>{if(n.id===e)return;let i=(n.dataset.category||``).split(` `);t.some(e=>e&&i.includes(e))&&r.push(n)}),r.length<4&&n.forEach(t=>{t.id!==e&&!r.includes(t)&&r.length<6&&r.push(t)}),r.slice(0,6)}function g(e){return`${window.location.pathname.includes(`/pages/`)?`./`:`./pages/`}comercio.html?id=${encodeURIComponent(e)}`}function _(){return`
  ...
  `}function y(e){let t=m(e.stock),n=e.stock<=0,r=(e.shop||`?`).charAt(0).toUpperCase(),a=h(e.id,e.categories||[]),o=``;a.forEach(e=>{let t=d(e.querySelector(`.product-card__name`)?.textContent?.trim()||``),n=d(e.querySelector(`.product-card__price`)?.textContent||``),r=encodeURI(e.querySelector(`.product-card__image img`)?.getAttribute(`src`)||``),i=d(e.id||``);o+=`
    ...
    `:`<span class="pm-rating__count">Todavía no tiene reseñas</span>`,g=``;return e.variants.length>0&&(g=`
      <div class="pm-variants">
        ...
        <ul class="pm-variants__list" style="list-style:none; padding:0; margin:0; display:flex; flex-direction:column; gap:0.3rem;">
          ${e.variants.map(e=>`<li style="font-size:0.85rem; color:var(--bl-text-secondary);">${d(e.name)} — ${i(e.price)} (${e.stock>0?`stock: ${e.stock}`:`sin stock`})</li>`).join(``)}
        </ul>
  ...
  `}var b=null,x=null,S=null;function C(e){let t=(history.state?.pmDepth||0)+1;history.pushState({pmProduct:e,pmDepth:t},``,location.href)}function w(){let e=history.state?.pmDepth||0;e>0?history.go(-e):E()}window.addEventListener(`popstate`,e=>{let t=e.state?.pmProduct,n=t&&document.getElementById(t);n?T(n,{skipHistoryPush:!0}):b&&E()});async function T(e,{skipHistoryPush:t=!1}={}){if(!e.id){console.error(`Se intentó abrir el modal de un producto sin id real (UUID)`);return}b&&E(),x=document.activeElement;let n=e.id,r=(e.dataset.category||``).split(` `);t||C(n);let i=document.createElement(`div`);i.className=`product-modal-overlay`,i.id=`pm-overlay`,i.innerHTML=_(),document.body.appendChild(i),b=i;let a=window.innerWidth-document.documentElement.clientWidth;document.documentElement.style.setProperty(`--scrollbar-width`,a+`px`),document.body.classList.add(`modal-open`),i.offsetHeight,requestAnimationFrame(()=>i.classList.add(`is-open`)),i.addEventListener(`click`,e=>{(e.target.closest(`#pm-close-back`)||e.target.closest(`#pm-close-btn`))&&w()}),S=e=>{e.key===`Escape`&&w()},document.addEventListener(`keydown`,S);try{let e=await f(n);if(e.categories=r,b!==i)return;i.innerHTML=y(e),D(i,e);let t=i.querySelector(`.pm-stock__fill`);if(t){let n=e.stock<=0?`0%`:``;t.style.width=`0%`,requestAnimationFrame(()=>{requestAnimationFrame(()=>{t.style.width=n})})}}catch(t){if(console.error(`Error al cargar el producto en el modal:`,t),b!==i)return;i.innerHTML=v(),i.querySelector(`#pm-retry-btn`)?.addEventListener(`click`,()=>T(e))}}function E(){if(!b)return;b.classList.remove(`is-open`),document.body.classList.remove(`modal-open`),document.documentElement.style.removeProperty(`--scrollbar-width`);let e=b;b=null,S&&=(document.removeEventListener(`keydown`,S),null),setTimeout(()=>{e.remove()},400),x&&=(x.focus(),null)}function D(e,i){let l=e.querySelector(`.product-modal`),d=e.querySelector(`#pm-gallery`);d?.addEventListener(`click`,e=>{e.target.closest(`.pm-gallery__fav`)||d.classList.toggle(`is-zoomed`)});let f=e.querySelector(`#pm-gallery-img`);if(e.querySelectorAll(`.pm-thumb`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.thumbSrc;f&&n&&(f.src=n),e.querySelectorAll(`.pm-thumb`).forEach(e=>e.classList.remove(`is-active`)),t.classList.add(`is-active`)})}),i.shopId){let t=()=>{window.location.href=g(i.shopId)},n=e.querySelector(`#pm-shop-link`);n?.addEventListener(`click`,t),n?.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),t())}),e.querySelector(`#pm-view-shop-btn`)?.addEventListener(`click`,t)}let p=e.querySelector(`#pm-fav-btn`);if(p&&i.id){let e=p.querySelector(`i`),n=!1;a().then(t=>{n=t.includes(i.id),p.classList.toggle(`is-active`,n),e.classList.toggle(`fa-solid`,n),e.classList.toggle(`fa-regular`,!n)}),p.addEventListener(`click`,async()=>{let r=n;n=!r,p.classList.toggle(`is-active`,n),e.classList.toggle(`fa-solid`,n),e.classList.toggle(`fa-regular`,!n),t(n?`Agregado a favoritos`:`Eliminado de favoritos`,n?`success`:`default`);try{await c(i.id,r)}catch(e){console.error(`Error al actualizar favoritos:`,e)}})}let m=e.querySelector(`#pm-qty-value`),h=e.querySelector(`#pm-qty-minus`),_=e.querySelector(`#pm-qty-plus`);function v(){let e=parseInt(m.value,10)||1;h.disabled=e<=1,_.disabled=e>=i.stock}h?.addEventListener(`click`,()=>{let e=parseInt(m.value,10)||1;e>1&&(m.value=e-1),v()}),_?.addEventListener(`click`,()=>{let e=parseInt(m.value,10)||1;e<i.stock&&(m.value=e+1),v()}),m?.addEventListener(`change`,()=>{let e=parseInt(m.value,10)||1;e=Math.max(1,Math.min(e,i.stock)),m.value=e,v()});let y=e.querySelector(`#pm-add-cart`);y?.addEventListener(`click`,()=>{let e=parseInt(m?.value,10)||1,a=s(i.priceOldText),c=o(),l=c.find(e=>e.id===i.id);l?l.qty+=e:c.push({id:i.id,name:i.name,shop:i.shop,price:i.price,priceOld:a||null,image:i.imgSrc,qty:e}),r(c),n(),y.classList.add(`pm-btn--added`);let u=y.innerHTML;y.innerHTML=`<i class="fa-solid fa-check"></i> ¡Agregado!`,setTimeout(()=>{y.classList.remove(`pm-btn--added`),y.innerHTML=u},1800),t(`${i.name} agregado al carrito (x${e})`,`success`)}),e.querySelector(`#pm-buy-now`)?.addEventListener(`click`,()=>{let e=parseInt(m?.value,10)||1,t=s(i.priceOldText),a=o(),c=a.find(e=>e.id===i.id);c?c.qty+=e:a.push({id:i.id,name:i.name,shop:i.shop,price:i.price,priceOld:t||null,image:i.imgSrc,qty:e}),r(a),n(),E();let l=window.location.pathname.includes(`/pages/`);window.location.href=l?`./carrito.html`:`./pages/carrito.html`}),e.querySelector(`#pm-share-btn`)?.addEventListener(`click`,async()=>{let e={title:i.name,text:`Mirá ${i.name} en Baradero Local por ${i.priceText}`,url:window.location.href};try{navigator.share?await navigator.share(e):(await navigator.clipboard.writeText(`${e.text} — ${e.url}`),t(`¡Link copiado al portapapeles!`,`success`))}catch(e){if(e.name!==`AbortError`)try{await navigator.clipboard.writeText(window.location.href),t(`¡Link copiado!`,`success`)}catch{}}});let b=e.querySelectorAll(`.pm-tab`),x=e.querySelectorAll(`.pm-tab-content`),S=!1;b.forEach(t=>{t.addEventListener(`click`,()=>{b.forEach(e=>{e.classList.remove(`is-active`),e.setAttribute(`aria-selected`,`false`)}),x.forEach(e=>e.classList.remove(`is-visible`)),t.classList.add(`is-active`),t.setAttribute(`aria-selected`,`true`);let n=t.dataset.tab,r=e.querySelector(`[data-tab-content="${n}"]`);if(r&&r.classList.add(`is-visible`),n===`reviews`&&!S){S=!0;let t=e.querySelector(`#pm-reviews-container`);t&&u(t,`product`,i.id)}})}),e.querySelectorAll(`.pm-related-card`).forEach(e=>{let t=()=>{let t=e.dataset.relatedId,n=document.getElementById(t);n&&(E(),setTimeout(()=>T(n),450))};e.addEventListener(`click`,t),e.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),t())})});let C=e.querySelector(`.pm-related__scroll`);C&&C.addEventListener(`wheel`,e=>{Math.abs(e.deltaY)<=Math.abs(e.deltaX)||(C.scrollLeft+=e.deltaY,e.preventDefault())},{passive:!1});let w=l.querySelectorAll(`button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])`),D=w[0],O=w[w.length-1];l.addEventListener(`keydown`,e=>{e.key===`Tab`&&(e.shiftKey?document.activeElement===D&&(e.preventDefault(),O.focus()):document.activeElement===O&&(e.preventDefault(),D.focus()))}),setTimeout(()=>D?.focus(),100)}function O(){document.querySelectorAll(`.products__grid`).forEach(e=>{e.addEventListener(`click`,e=>{if(e.target.closest(`.product-card__add, .product-card__wishlist`))return;let t=e.target.closest(`.product-card`);t&&(e.preventDefault(),T(t))})})}typeof window<`u`&&(window.initProductModal=O,window.openProductModal=T,window.closeProductModal=E);