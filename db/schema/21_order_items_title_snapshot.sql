-- F2-06 (A113-178) — historial de pedidos: sacar el título de cada producto
-- comprado de una foto fija en `order_items`, no de un join en vivo a
-- `products`.
--
-- Motivo: `order_items.price` ya es un snapshot del precio al momento de la
-- compra (no se recalcula si el producto cambia de precio después), pero el
-- título del producto no lo era — `perfil.html` lo iba a leer con un join a
-- `products(title)`. Problema: `products` solo tiene SELECT público para
-- `is_active = true` (o el propio vendedor); si el vendedor después
-- desactiva o borra el producto, el cliente dejaría de poder ver qué
-- compró en su propio historial (RLS se aplica también dentro de los joins
-- de PostgREST). Un recibo de compra no debería poder "desaparecer" así.
--
-- Se agrega `order_items.title` (snapshot, igual que `price`) y se actualiza
-- `create_order` para completarlo desde `_order_cart_items` (que ya trae el
-- título leído de `products` en el momento del checkout — no hace falta una
-- query extra).
