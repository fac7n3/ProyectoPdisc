...
- [x] **F12-07** 🟡 Cupones visibles públicamente. `renderActiveCoupons()` (nuevo, `cart-utils.js`) — la RLS `coupons_select_public` (F0) ya dejaba leer cualquier cupón activo/no vencido a `anon`, era 100% frontend. Sección "Cupones activos" en home.html (informativa, clic copia el código) y en el desplegable de cupón de carrito.html (clic completa el input y aplica). Muestra a qué tienda aplica (o "Todo Baradero Local" si es global, F12-03) y vencimiento si tiene.
- [x] **F12-08** 🟡 Calificar al repartidor. `reviews.target_type` (F7-01) solo aceptaba `product`/`store` — CHECK ampliado a `repartidor` (migración 44, sin cambios de RLS: `reviews_insert_own` ya era genérica). `perfil.js` ("Mis compras"): widget compacto (estrellas + submit, reusa `submitReview()`) que aparece cuando la entrega está `delivered`; se califica a la persona, no al pedido (mismo `unique(target_type,target_id,client_id)` que producto/tienda — reentregas del mismo repartidor actualizan la reseña). `repartidor.js`: línea "Mi calificación" en el dashboard (reusa `fetchReviewsSummary()` sin cambios).
- [x] **F12-09** 🟡 Aviso de "volvió el stock". Tabla `stock_alerts` (migración 45) + trigger `notify_stock_alerts` en `products` (dispara solo en la transición `stock=0 → stock>0`, notifica a cada cliente con alerta pendiente vía `create_notification` y las marca `notified_at`). `producto.js`: si `stock<=0`, "Agregar al carrito" queda deshabilitado y aparece "Avisarme cuando vuelva el stock" (requiere sesión, a diferencia de favoritos no tiene modo invitado). Hallazgo de alcance: no existía NINGÚN estado "Agotado" en todo el sitio (home/search/comercio dejaban agregar al carrito productos con `stock=0` sin aviso — cubierto solo por la revalidación de F2-01/F4-02 al pagar) — a propósito se resolvió solo en la página de detalle (el lugar natural para esta acción puntual), no en las grillas.
- [x] **F12-10** 🟡 Panel de admin para `error_logs`. Sin migración (la tabla y su RLS `error_logs_select_admin` ya existían desde F1/A113-171) — 100% frontend. Sección nueva "Errores registrados" en `admin.js`/`admin.html`: fecha, usuario (resuelto vía `profiles.email` con una segunda consulta, mismo patrón que `phoneByClientId` de F12-05 — `error_logs.user_id` referencia `auth.users`, no `profiles`; "Invitado" si es null), mensaje y URL (truncados con `title` para ver completo al pasar el mouse), botón "Ver detalle" con el stack trace completo. Solo lectura (es telemetría de diagnóstico, no un flujo con estados) y limitado a los últimos 100.
- [x] **F12-11** 🟡 Panel de soporte/reclamos. Tabla `support_tickets` (migración 46, RLS: dueño o admin ven, dueño inserta, solo admin actualiza el estado) + trigger que notifica al autor cuando el admin cambia el estado. `support-utils.js` (nuevo, compartido): form "Contactar a soporte" + lista de "Mis reclamos" con badge de estado — integrado en `perfil.js` (tab nueva "Soporte"), `vender.js` y `repartidor.js` (misma sección en ambos dashboards). Sección nueva "Soporte / Reclamos" en `admin.js`/`admin.html`: lista cross-usuario con `<select>` de estado (abierto/en progreso/resuelto) por fila. Alcance a propósito acotado: sin hilo de respuesta en la app (el admin sigue respondiendo por email, ahora con contexto estructurado en vez de un mail genérico) — un hilo completo repetiría el chat de F7-02 para un caso de uso distinto.
- [x] **F12-12** 🟢 Log de auditoría de admin. Tabla `admin_audit_log` (migración 47) + trigger genérico `log_admin_action` (mismo patrón que `set_updated_at`) adjunto a `stores`/`seller_requests`/`delivery_requests`/`profiles`/`products`/`categories`/`coupons`/`reviews`/`support_tickets`/`orders` — solo registra la fila cuando quien ejecuta la escritura es realmente admin (chequea `auth.jwt()->>'role'` del que llama, no cambia por correr dentro de una función `SECURITY DEFINER`), así que un vendedor editando su propia tienda o un cliente su propio perfil no generan ruido. Sección nueva "Log de auditoría" en `admin.js`/`admin.html`. **Hallazgo operativo**: ningún usuario real en producción tiene `app_metadata.role='admin'` seteado todavía — el panel de admin nunca fue accedido con una cuenta real, solo simulado en SQL. Hay que asignarlo a mano desde el dashboard de Supabase (Authentication → Users → editar `raw_app_meta_data`) para poder usarlo.
...
- [x] **F12-14** 🟢 Vencimiento de ofertas. `products.offer_expires_at` (migración 48, tipo `date` no `timestamptz` — el vendedor elige un día, no una hora exacta; "vence el 30" debe seguir mostrando la oferta durante todo el 30). `buildPriceRow()` (`cart-utils.js`) ignora el precio de oferta si ya venció — sin tocar la columna, el vendedor no tiene que acordarse de sacarla a mano. `search.js`: el filtro "Ofertas" (F9-03) ahora también excluye las vencidas. Input nuevo "Vencimiento de la oferta (opcional)" en el form de producto de `vender.js`/`vender.html`, solo se guarda si hay un precio de oferta cargado (evita una fecha huérfana sin oferta real). Verificado con casos sintéticos vía `preview_eval` (sin oferta / oferta sin vencimiento / vigente / vence hoy / vencida ayer) — los 5 se comportan como corresponde.
- [x] **F12-15** 🟢 Onboarding para vendedor recién aprobado. Sin migración — 100% frontend, basado en estado real (no una preferencia de "descartado" guardada en ningún lado). Banner "¡Bienvenido a Baradero Local!" en `vender.js`/`vender.html` con un checklist de 2 pasos (completar el perfil del comercio / publicar el primer producto), cada uno clickeable (scrollea al form de perfil o abre el form de alta de producto). Desaparece solo apenas se cumplen los dos — se recalcula en cada `fetchProducts()` y al guardar el perfil, sin esperar a un reload.
- [x] **F12-16** 🟢 Multi-usuario por comercio. Diseño delegado explícitamente por el usuario. Tabla `store_staff` (migración 49) + RPC `add_store_staff(store_id, email)` (busca la cuenta por email vía `profiles`, solo el dueño puede llamarla). Paridad operativa total para el empleado (productos, pedidos, comprobantes de transferencia) vía policies **aditivas** en `products`/`orders`/`order_items`/`payment_proofs` — nunca se tocó una policy existente del dueño. El empleado **no** puede: editar el perfil del comercio, gestionar cupones, ni gestionar otros empleados (decisiones financieras/de acceso, quedan solo para el dueño). `vender.js`: `checkSellerState`/`loadDashboard` ahora also revisan `store_staff` (no hace falta rol propio de vendedor); secciones exclusivas del dueño se ocultan para el empleado; nueva sección "Empleados" (alta por email + quitar acceso). Verificado con `BEGIN;...ROLLBACK;`: 9+ escenarios (alta, auto-alta rechazada, alta por un extraño rechazada, empleado inserta/edita productos, extraño no ve un producto inactivo ajeno, empleado no puede editar el perfil de la tienda, empleado no puede quitarse a sí mismo de `store_staff`, dueño sí puede quitarlo, empleado ve/actualiza pedidos de su tienda).
- [x] **F12-17** 🟢 Roles de admin granulares. Diseño delegado explícitamente por el usuario. Un solo rol nuevo, `moderador` (vive solo en `app_metadata.role`, como `admin` — sin tocar el enum `app_role`/`profiles.role`), acotado a las dos tareas de "confianza y seguridad" delegables sin exposición financiera: moderar reseñas reportadas (F7-03) y gestionar reclamos de soporte (F12-11). Todo lo demás (comercios, repartidores, cupones, categorías, comprobantes, métricas, logs) sigue siendo exclusivo de `admin`. **Bug encontrado y corregido en el camino**: `guardPage({requireRole})` comparaba contra `profiles.role`, no `app_metadata.role` (lo que de verdad evalúa el RLS) — como ningún flujo de la app sincroniza `profiles.role` a `'admin'` (a diferencia de vendedor/repartidor), el panel de admin era **inaccesible incluso para una cuenta con `app_metadata.role='admin'` bien seteada**. Corregido en `auth-utils.js` + soporte para un array de roles permitidos. 4 policies nuevas aditivas (`reviews`/`support_tickets`, select+update) + `log_admin_action()` (F12-12) ampliada para auditar también las acciones de moderador. `admin.html`: 14 secciones etiquetadas con `data-section` + `admin.js` oculta las 12 exclusivas de admin cuando el rol es moderador. Verificado con `BEGIN;...ROLLBACK;`: moderador ve/oculta reseñas reportadas y gestiona reclamos (queda auditado), no puede tocar categorías; acceso del admin sin cambios.
